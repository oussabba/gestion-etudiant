...
namespace MiniProjetDotNet.user_controls
{
    public partial class GestionEtudiantUC : UserControl
    {
        ...
        private void modiferButton_Click(object sender, EventArgs e)
        {
            if (numEtudiantTextBox.Text == "" || nomEtudiantTextBox.Text == "" || prenomEtudiantTextBox.Text == "" || dateNaissanceDateTimePicker.Value == null)
            {
                MessageBox.Show("Tous les champs sont requis !");
            ...
            else
            {
                using (SqlConnection cnx = ConnectionDB.Cnx())
                {
                    try
                    {
                        ...
                        {
                            reader.Close();
                            SqlCommand insertCmd = new SqlCommand("UPDATE Etudiant SET Num_Etu=@numEtudiant, Nom_Etu=@nomEtudiant,Prenom_Etu=@prenomEtudiant,DateN_Etu=@dateNaissance)", cnx);
                            SqlParameter numEtudParameter2 = new SqlParameter();
                            numEtudParameter2.ParameterName = "@numEtudiant";
                            numEtudParameter2.Value = numEtudiantTextBox.Text;
                            SqlParameter nomEtudParameter = new SqlParameter();
        ...
        {
            if (numEtudiantTextBox.Text == "")
            {
                MessageBox.Show("Il faut saisir un numéro d'inscription !");
            }
            else
            {
                using (SqlConnection cnx = ConnectionDB.Cnx())
                {
                    ...
                    {
                        cnx.Open();
                        SqlCommand cmd = new SqlCommand("DELETE FROM Etudiant WHERE Num_Etu=@numEtudiant", cnx);
                        SqlParameter numEtudParameter = new SqlParameter();
                        numEtudParameter.ParameterName = "@numEtudiant";
                        numEtudParameter.Value = numEtudiantTextBox.Text;
                        cmd.Parameters.Add(numEtudParameter);
                        ...
                        {
                            MessageBox.Show("Etudiant supprimé avec succés");
                        }
                    }
                    catch (SqlException ex)
                    {
        ...
        private void moyenneButton_Click(object sender, EventArgs e)
        {
            if (numEtudiantTextBox.Text == "")
            {
                MessageBox.Show("Veuillez saisir un numéro valide !");
            ...
            else
            {
                using (SqlConnection cnx = ConnectionDB.Cnx())
                {
                    ...
                    {
                        cnx.Open();
                        SqlCommand cmd = new SqlCommand("SELECT COUNT(*) AS nombreModules,AVG(note) AS moyenne FROM Notes WHERE Num_Etu=@numEtudiant", cnx);
                        SqlParameter numEtudParameter = new SqlParameter();
                        numEtudParameter.ParameterName = "numEtudiant";
                        numEtudParameter.Value = numEtudiantTextBox.Text;
                        cmd.Parameters.Add(numEtudParameter);
                        ...
                        if (reader.Read())
                        {
                            if ((int)reader["nombreModules"] == 6)
                            {
                                MessageBox.Show("La moyenne de l'étudiant est : "+ reader["moyenne"].ToString());
                            }
                            else
                            {
                                MessageBox.Show("Il manque une note n'est pas saisie !");
                            }
                        }
                        else
                        {
